...
(function () {
  ...
  function initDogRingBearer() {
    ...
    $('#dogContent').textContent = CONFIG.dogRingBearer.content;

    const container = $('#dogPhotos');
    const imagePaths = ['images/dog/1.jpg', 'images/dog/2.jpg'];
    const loadedImages = [];
    ...
    imagePaths.forEach((src, i) => {
      ...
      probe.onload = function () {
        ...
        const div = document.createElement('div');
        div.className = 'dog__photo-item animate-item';
        div.setAttribute('data-animate', 'fade-up');
        div.innerHTML = `<img src="${src}" alt="강아지 화동 사진 ${i + 1}" loading="lazy">`;
        div.addEventListener('click', () => {
          const available = loadedImages.filter(Boolean);
          openPhotoModal(available, available.indexOf(src));
        });

        // 순서를 유지하기 위해 index 기반으로 삽입
        if (i === 0 || container.children.length === 0) {
          container.appendChild(div);
        } else {
          container.appendChild(div);
        }
