...
(function () {
  ...
  /* ═══════════════════════════════════════════
     Dog Ring Bearer Section (강아지 화동)
     ═══════════════════════════════════════════ */

  function initDogRingBearer() {
    $('#dogTitle').textContent = CONFIG.dogRingBearer.title;

    // content를 \n\n 기준으로 분리해 각 줄별로 텍스트 + 이미지 렌더링
    const lines = CONFIG.dogRingBearer.content.split('\n\n').map(s => s.trim()).filter(Boolean);
    const container = $('#noticeItems');
    const loadedImages = [];

    lines.forEach((line, i) => {
      const imgSrc = `images/notice/${i + 1}.jpg`;

      // 텍스트 블록
      const textEl = document.createElement('p');
      textEl.className = 'notice__item-text animate-item';
      textEl.setAttribute('data-animate', 'fade-up');
      ...
      probe.onload = function () {
        loadedImages[i] = imgSrc;

        const photoWrap = document.createElement('div');
        photoWrap.className = 'notice__item-photo animate-item';
        photoWrap.setAttribute('data-animate', 'fade-up');
        photoWrap.innerHTML = `<img src="${imgSrc}" alt="안내 사진 ${i + 1}" loading="lazy">`;
        photoWrap.addEventListener('click', () => {
          const available = loadedImages.filter(Boolean);
          openPhotoModal(available, available.indexOf(imgSrc));
        });

        // textEl 바로 다음에 삽입 (순서 보장)
        textEl.insertAdjacentElement('afterend', photoWrap);
      };
      probe.src = imgSrc;
    });
  }
